...
int main(int argc, char **argv) {
    ...
    if (numtasks == 1) {
        fprintf(stderr, "Error: The number of processes must be greater than 1 for parallel computation.\n");
        MPI_Finalize();
        exit(EXIT_FAILURE);
    }
    ...
    if (taskid == 0) {
        ...
        for (i = 1; i <= numworkers; i++) {
            ...
            MPI_Recv(&c[offset * N], rows * N, MPI_DOUBLE, source, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
