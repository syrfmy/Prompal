...
void FoxMultiply(int n, GridStructure *grid, int **a, int **b, int **c, double *computation_time, double *communication_time) {
    int **temp_a, *buff, stage, root, submat_dim, src, dst;
    MPI_Status status;
    clock_t computation_start, computation_end, communication_start, communication_end;
    ...
    for (stage = 0; stage < grid->dim; stage++) {
        ...
        if (root == grid->col) {
            ...
            communication_start = clock();
            MPI_Bcast(buff, submat_dim * submat_dim, MPI_INT, root, grid->row_comm);
            communication_end = clock();
            *communication_time += (double)(communication_end - communication_start) / CLOCKS_PER_SEC;
    ...
    *computation_time = (double)(computation_end - computation_start) / CLOCKS_PER_SEC;
}
...
int main(int argc, char *argv[]) {
    ...
    FoxMultiply(MATRIX_SIZE, &grid, local_a, local_b, local_c, &computation_time, &communication_time);
    MPI_Barrier(grid.grid_comm);
    if (grid.rank == 0)
    {
        finish = clock();
        clock_t result_time = finish - start;
        std::cout << "Total Time: " << double(result_time)/CLOCKS_PER_SEC << std::endl;
